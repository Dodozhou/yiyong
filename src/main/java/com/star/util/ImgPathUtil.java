...
public class ImgPathUtil {
    ...
    public String saveImgs(HttpServletRequest request, MultipartFile image, String category) throws IOException {
        String randomDir=generateRandomDir();
        String fileName=generateRandomFileName(image.getOriginalFilename());
        File dir=new File(request.getSession().getServletContext().getRealPath("/uploads/"+category+randomDir));
        //File dir=new File("/opt/springboot/uploads/"+category+randomDir);
        if(!dir.exists()){
            dir.mkdirs();
        }
        String finalFileName=dir.getAbsolutePath()+"/"+fileName;
        image.transferTo(new File(finalFileName));
        return "/uploads/"+category+randomDir+"/"+fileName;
    }
